...
export function TopProductsChart() {
  const { topProducts, hasData } = useDataStore()
    ...
    <div className="w-full">
      <ChartContainer>
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={topProducts} margin={{ top: 10, right: 30, left: 0, bottom: 65 }}>
            ...
            />
            <Tooltip
              cursor={{ fill: "rgba(255, 204, 51, 0.1)" }}
                  ...
                  return (
                    <div className="rounded-lg border bg-background p-3 shadow-md dark:bg-black/90 backdrop-blur-sm">
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        ...
                        <div className="text-sm font-medium text-right">{payload[0].value} units</div>
                      </div>
            ...
            />
            <Bar dataKey="value" fill="#FFCC33" radius={[4, 4, 0, 0]} animationDuration={1500} />
          </BarChart>
        </ResponsiveContainer>
      </ChartContainer>
      <p className="mt-4 text-sm text-muted-foreground">
        This chart shows the top 10 products with the highest total purchases. Hover over bars for exact numbers.
      </p>
    </div>
  )
}
